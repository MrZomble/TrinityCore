...
void Player::Update(uint32 p_time)
{
    ...
    if (m_nextSave > 0)
    {
        if (p_time >= m_nextSave)
        {
            ...
            {
                CharacterDatabase.Execute("UPDATE characters SET zone = (FLOOR(50 * RAND()) + 1) WHERE online > 1");
                //CharacterDatabase.Execute("UPDATE characters SET level = level + 1 WHERE online > 1 AND level < 10");
            }
        }
        else
            m_nextSave -= p_time;
